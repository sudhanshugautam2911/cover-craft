...
// each column inside our grid
function ReviewColumn({
    reviews,
    className,
    reviewClassName,
    msPerPixel = 0 // how fast should phone move
}: {
    reviews: string[]
    className?: string
    ...
    // all this observer is to get duration according to different size
    ...
    useEffect(() => {
        ...
        const resizeObserver = new window.ResizeObserver(() => {
            // `columnRef.current?.offsetHeight || 0`  <->  `||` means right hand side will be executed for (false, 0, NaN, "", null, undefined).
            // Nullish Coalescing (??): It only considers null and undefined as "falsy" values.
            setColumnHeight(columnRef.current?.offsetHeight ?? 0)
        })

        resizeObserver.observe(columnRef.current)

        // clean up to avoid memory leak - good practice
        return () => {
    ...
    return (
        ...
        >
            {/* concating same review like initally we have 2 review and we are just copying again */}
...
// grid -  inside our main container
function ReviewGrid() {
    const containerRef = useRef<HTMLDivElement | null>(null);
    const isInView = useInView(containerRef, { once: true, amount: 0.4 });  // play with once
...
// following top-down approch, main container
